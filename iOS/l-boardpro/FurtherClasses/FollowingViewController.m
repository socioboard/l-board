...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(createUI) name:@"loadFollowing" object:nil];
...
//create Table to show following company.
-(void)createTableView{
    if (followingTable) {
        followingTable=nil;
    }
    followingTable=[[UITableView alloc]initWithFrame:CGRectMake(0, 0, windowSize.width, windowSize.height-50)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (companyDetailVC) {
     ...
     companyDetailVC=[[CompanyDetailViewController alloc]init];
    companyDetailVC.companyId=[companyId objectAtIndex:indexPath.row];
...
// get all user following companies.
